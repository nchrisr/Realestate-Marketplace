...
contract Ownable {
    ...
    modifier validAddress(address theAddress){
        require(address(theAddress) == theAddress, "The address is not valid.");
        _;
    }
...
contract Pausable is Ownable {
    ...
    function setContractPaused(bool paused) public onlyOwner{
        _paused = paused;
        if (_paused){
...
contract ERC721 is Pausable, ERC165 {
    ...
    modifier addressIsValid(address theAddress){
        require(theAddress != address(0) && address(theAddress) == theAddress, "Address is not valid.");
        _;
    }

    modifier addressIsOwner(address ownerAddress, uint256 tokenId){
        require(ownerAddress == _tokenOwner[tokenId], "Address does not own token.");
        _;
    }

    modifier addressNotOwner(address theAddress, uint256 tokenId){
        require (theAddress != _tokenOwner[tokenId], "Address already owns token.");
        _;
    }

    modifier senderIsAuthorized(address owner){
        require(msg.sender == getContractOwner() || isApprovedForAll(owner, msg.sender), "Sender does not have authorization.");
        _;
    }
    ...
    function approve(address to, uint256 tokenId)
        public
        addressNotOwner(to, tokenId)
        senderIsAuthorized(_tokenOwner[tokenId])
    {
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function _setTokenURI(uint256 tokenId) internal tokenExists(tokenId)
    {
...
contract ERC721MintableRealEstate is ERC721Metadata("Real Estate Tokens",
                                                    "RET",
